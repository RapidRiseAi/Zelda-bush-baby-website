...
function setupSheets() {
  const config = getConfig_({ requireOwnerEmail: false });
...
function doPost(e) {
  try {
    const config = getConfig_({ requireOwnerEmail: true });
...
function getConfig_(options) {
  ...
  const configuredSheetId = props.getProperty('SHEET_ID');
  const fallbackActiveSheetId = tryGetActiveSpreadsheetId_();
  const sheetId = configuredSheetId || fallbackActiveSheetId;
  const ownerEmail = props.getProperty('OWNER_EMAIL');

  if (!sheetId) {
    throw new Error(
      'Missing script property: SHEET_ID. Add it in Apps Script > Project Settings > Script properties.'
    );
  }
  if (settings.requireOwnerEmail && !ownerEmail) {
    throw new Error(
      'Missing script property: OWNER_EMAIL. Add it in Apps Script > Project Settings > Script properties.'
    );
  }
...
function tryGetActiveSpreadsheetId_() {
  try {
